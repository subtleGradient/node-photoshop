  ...
  if (evalInAfterEffects.NAME == null) {
    try {
      ;(function(error, Applications){
        ...
        var runningSupportedApps = supportedApps.filter(function (appName) {
          return Applications.indexOf(appName) != -1;
        });

        if (runningSupportedApps.length) {
          evalInAfterEffects.NAME = runningSupportedApps[0];
        }
