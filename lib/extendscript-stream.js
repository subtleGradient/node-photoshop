...
function createSingleUseTCPStream(config, callback){
  ...
  server.on('error', function(error){
    if (exports.debug) console.log(error)
    if (error.code == 'EADDRINUSE') {
      server.listen(++_port, _host)
      if (exports.debug) console.log('EADDRINUSE, trying', _port)
      return
    }
    outputPassThrough.emit('error', error)
  })
  server.on('connection', function(client){
    if (exports.debug) console.log('connection from client')
  ...
  server.on('close', function(){
    if (exports.debug) console.log('server close')
...
function jsxStream(fn, args){
  var outputPassThrough = createSingleUseTCPStream(function(error, address){
    ...
    result.on('data', function(data){ if (exports.debug) console.log('jsxStream psEval data', String(data)) })
