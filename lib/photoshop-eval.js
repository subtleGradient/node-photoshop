  ...
  if (evalInPhotoshop.NAME == null) {
    try {
      ;(function(error, Applications){
        if (Applications.indexOf('Adobe Photoshop CC 2014') != -1) evalInPhotoshop.NAME = "Adobe Photoshop CC 2014";
...
function evalInPhotoshop(fn, args){
  ...
  readable._read = function(size){
    ...
    cliArgs.push('-e',   'set JS to (item 1 of argv)')
    cliArgs.push('-e',   'tell application "' + evalInPhotoshop.getName() + '" to do javascript JS' + (module.exports.debugger ? ' show debugger on runtime error' : ''))
