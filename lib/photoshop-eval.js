...
function evalInPhotoshop(fn, args){
  ...
  readable._read = function(size){
    ...
    cliArgs.push("-e",   'tell application "Adobe Photoshop CS6" to do javascript (item 1 of argv)' + (module.exports.debug ? ' show debugger on runtime error' : ''))
    ...
    lockFile.lock(lockPath, lockOptions, function(error, fileDescriptor){
      if (error){
        if (exports.debug) {debug.error = error; console.warn('NOT LOCKED', debugInfo)}
      ...
      child.on('exit', function(code){
        ...
        if (code) readable.emit('error', _error);
      })
